...
fn main() {
    App::new()
        ...
        .add_systems(Update, (sync_panels, sync_volume_text, sync_quality_buttons))
        .run();
}
...
enum MenuScreen { #[default] Main, Audio, Graphics }

#[derive(Resource)]
struct AudioSettings { volume: i32, muted: bool }
impl Default for AudioSettings { fn default() -> Self { Self { volume: 7, muted: false } } }

#[derive(Resource, Default, PartialEq, Eq, Clone, Copy, Debug)]
enum GraphicsQuality { Low, #[default] Medium, High }

#[derive(Resource, Default)]
struct GraphicsSettings { quality: GraphicsQuality }

// ── Marker components ─────────────────────────────────────────────────────────

#[derive(Component)] struct MainPanel;
#[derive(Component)] struct AudioPanel;
#[derive(Component)] struct GraphicsPanel;
#[derive(Component)] struct VolumeText;
#[derive(Component)] struct QualityButton(GraphicsQuality);
...
fn panel_shell(ui: &mut UIBuilder, f: impl FnOnce(&mut UIBuilder)) {
    ui.display_flex().flex_column()
        .align_items_center().gap_px(16.0)
        .padding_all_px(40.0)
...
fn build_main_panel(ui: &mut UIBuilder, _theme: &LavaTheme) {
    ui.with_child(|panel| {
        ...
        panel_shell(panel, |p| {
            ...
            p.add_text_child("Main Menu", Some(TextStyle::size_color(18.0, Color::srgb(0.7, 0.7, 0.9))));

            p.add_button_observe("  Play  ", |btn| { btn.size_px(260.0, 56.0); },
                |_: On<Activate>| info!("▶ Play!"));

            p.add_button_observe("  Audio  ", |btn| { btn.size_px(260.0, 56.0); },
                |_: On<Activate>, mut s: ResMut<MenuScreen>| { *s = MenuScreen::Audio; });

            p.add_button_observe("  Graphics  ", |btn| { btn.size_px(260.0, 56.0); },
                |_: On<Activate>, mut s: ResMut<MenuScreen>| { *s = MenuScreen::Graphics; });
...
fn build_audio_panel(ui: &mut UIBuilder, _theme: &LavaTheme) {
    ui.with_child(|panel| {
        ...
        panel_shell(panel, |p| {
            p.add_text_child("Audio Settings", Some(TextStyle::size_color(32.0, Color::WHITE)));

            // Volume row: label  [ - ]  value  [ + ]
            p.add_row(|row| {
                row.gap_px(12.0).align_items_center();

                row.add_text_child("Volume", Some(TextStyle::size_color(20.0, Color::srgb(0.8, 0.8, 0.8))));

                row.add_button_observe(" − ", |btn| { btn.size_px(40.0, 40.0); },
            ...
            // Mute toggle
            p.add_button_observe("  Toggle Mute  ", |btn| { btn.size_px(220.0, 48.0); },
                |_: On<Activate>, mut s: ResMut<AudioSettings>| {
                    s.muted = !s.muted;
                    info!("Muted: {}", s.muted);
                });
...
fn build_graphics_panel(ui: &mut UIBuilder, _theme: &LavaTheme) {
    ui.with_child(|panel| {
        ...
        panel_shell(panel, |p| {
            p.add_text_child("Graphics Settings", Some(TextStyle::size_color(32.0, Color::WHITE)));
            p.add_text_child("Quality", Some(TextStyle::size_color(18.0, Color::srgb(0.7, 0.7, 0.9))));
...
fn back_button(p: &mut UIBuilder) {
    p.add_button_observe("← Back",
        |btn| { btn.size_px(140.0, 44.0).bg_color(Color::srgb(0.2, 0.2, 0.35)); },
        |_: On<Activate>, mut s: ResMut<MenuScreen>| { *s = MenuScreen::Main; });
}
...
fn sync_panels(
    ...
    mut graphics_q: Query<&mut Node, (With<GraphicsPanel>, Without<MainPanel>,  Without<AudioPanel>)>,
) {
    if !menu.is_changed() { return; }
    let show = |display: &mut Node, visible: bool| {
        display.display = if visible { Display::Flex } else { Display::None };
    };
    if let Ok(mut n) = main_q.single_mut()     { show(&mut n, *menu == MenuScreen::Main); }
    if let Ok(mut n) = audio_q.single_mut()    { show(&mut n, *menu == MenuScreen::Audio); }
    if let Ok(mut n) = graphics_q.single_mut() { show(&mut n, *menu == MenuScreen::Graphics); }
}

fn sync_volume_text(audio: Res<AudioSettings>, mut texts: Query<&mut Text, With<VolumeText>>) {
    if !audio.is_changed() { return; }
    for mut t in &mut texts {
        **t = if audio.muted { "M".to_string() } else { audio.volume.to_string() };
    }
}
